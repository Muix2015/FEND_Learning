...
var Engine = (function(global) {
    ...
    // 遍历 enemies 数组判定碰撞事件，当碰撞发生时，弹窗警告并重置player对象
    function checkCollisions() {
        allEnemies.forEach( function(enemy) {
            if (enemy.isCollision) {
                console.log(enemy.isCollision);
                player.x = 202;
                player.y = 415;
                alert("Damn it!!!");
    ...
    function initEnemies() {
        setInterval(function(){
            let xPos = 0;
            let yPos = Math.round(Math.random()*2+1) * 83;
            let newEnemy = new Enemy(xPos, yPos);
            allEnemies.push(newEnemy);
        }, Math.round(Math.random()*2 + 1) *500 );
    }
